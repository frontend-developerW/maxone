...
import { FilterSvg, FilterWhite } from "../components/Svgs";
...
function Products() {
  ...
  const [activeType, setActiveType] = useState({id: Number(localStorage["type"]) || 2});
  ...
  const [category, setCategory] = useState({id: Number(localStorage["category"]) || 1});
  ...
  return (
    <div className="flex justify-between p-[7vw] pt-0 items-start">
      <div className={`md:bg-[#8cc5ff5b] md:h-auto h-[90vh] md:overflow-hidden overflow-y-scroll z-50 bg-[#c7e3ff] md:w-[25%] w-[90vw] md:left-auto left-[5vw] md:rounded-[2vw] rounded-[4vw] md:p-[3vw] md:pb-[3vw] pb-[26vw] p-[8vw] md:relative fixed transition-[.5s] md:top-0 md:z-0 ${filterView ? 'top-[10vh]' : 'top-[-200vh]'}`}>
        <div className="flex items-center md:gap-[1vw] gap-[4vw] relative">
          ...
          <div className="md:hidden" onClick={()=>setFilterView(!filterView)}>
            ...
            {categorys.map((item, i) => (
              <button
                ...
                className={` bg-[#fff] md:p-[.5vw] p-[2vw] md:px-[1vw] px-[5vw] rounded-[6vw] md:text-[.9vw] text-[4vw] ${
                  (category.id === item.id && "bg-[#004B99] text-[#ffffff]") ||
                  "text-[#004B99] "
                }`}
              >
          ...
          <div className="flex flex-wrap md:gap-[1vw] gap-[3vw] mt-[1vw]">
             {types.map((item, i) => (
              <button
                ...
                key={i}
                className={` bg-[#fff] md:p-[.5vw] p-[2vw] md:px-[1vw] px-[5vw] rounded-[6vw] md:text-[.9vw] text-[4vw] ${
                  (activeType.id === item.id && "bg-[#004B99] text-[#ffffff]") ||
                  "text-[#004B99] "
                }`}
              >
                {item?.[`name_${currentLang}`]}
      ...
      <div className="absolute top-[5vw] right-[10vw]" onClick={()=>setFilterView(!filterView)}>
        <FilterWhite />
      </div>
